...
bool RunFlutter(GLFWwindow* window,
  ...
  config.open_gl.gl_proc_resolver = [](void*, const char* symbolName) -> void* {
    auto fn = glfwGetProcAddress(symbolName);

    if (std::strncmp(symbolName, "eglQueryString", 14) == 0)
      return nullptr;

    int code = glfwGetError(NULL);
    if (code != GLFW_NO_ERROR) {
      std::cout << "Error resolving" << symbolName << std::endl;
      return nullptr;
    }
    std::cout << "Resolved " << symbolName << fn << std::endl;
